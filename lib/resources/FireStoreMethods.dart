...
class FireStoreMethods{
  final FirebaseFirestore _firestore= FirebaseFirestore.instance; //to access the firebase
...
Future<String> uploadPost(
  ...
  Uint8List file,
    ...
    try{
      String url_post =  await Storagetypes().UploadImageToStorage('Posts', file, true);
      String Id_post = const Uuid().v1(); //this sets a unique postId based on time
   ...
   Future<void> likePost(String postId, String uid, List likes)async{
      try{
        if(likes.contains(uid)){ //if post is already liked by a particular uid/user
          await _firestore.collection('posts').doc(postId).update({
            'Likes': FieldValue.arrayRemove([uid]) //array of users/uid
             //remove the like by current user from array
          });
        }
        else{ //else like the post and add uid of current user to the likes array
          await _firestore.collection('posts').doc(postId).update({
